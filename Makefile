...
vale_cmd ?= $(engine_cmd) run $(engine_opts) --volume "$${PWD}"/docs/modules:/pages:Z docker.io/vshn/vale:2.10.5.1 --minAlertLevel=error /pages
...
.PHONY: check
check: ## Run vale against the documentation to check writing style
	$(vale_cmd)
